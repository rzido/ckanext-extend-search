...
class ExtendSearchPlugin(plugins.SingletonPlugin,  DefaultTranslation):
    ...
    def before_search(self, search_params):
        ...
        if start_date and end_date:
            # Add a date-range query with the selected start and end dates into
            # the Solr facet queries.
    ...
    def after_search(self, search_params, search_results):
        ...
        orgs = ckan.logic.get_action('organization_list_for_user')(
            context, data_dict)
        ...
        # get all maintainers
        maintainers = [p[0] for p in meta.Session.query(
            distinct(Package.maintainer)) if p[0]]
